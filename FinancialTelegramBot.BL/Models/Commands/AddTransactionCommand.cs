    ...
    public class AddTransactionCommand : BuisnessLogicCommand, ICommand, IListener
    {
        ...
        public async Task GetUpdate(Update update)
        {
            ...
            if(_amount == 0)
            {
                ...
                var categories = await DataBaseConnector.CategoryService.GetAllCategoriesByUserIdAsync(user.Id);
                if (categories != null)
                {
            ...
            if(_categoryId == 0)
            {
                ...
                var accounts = await DataBaseConnector.AccountService.GetAllAccountsByUserIdAsync(user.Id);
            ...
            if(_accountId == 0)
            {
                ...
                if(_accountId == 0)
                {
                    ...
                    var currencies = await DataBaseConnector.CurrencyService.GetAllCurrenciesByUserIdAsync(user.Id);
